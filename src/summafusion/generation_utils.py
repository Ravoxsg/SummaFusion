...
class GenerationMixin:
    ...
    def _prepare_encoder_decoder_kwargs_for_generation(
        ...
        if encoder_kwargs["input_ids"].shape[1] > max_source_length:
            ...
            thresh = model_kwargs["max_source_length"]
            ...
            if model_kwargs["use_candidates"] == True:
                to_cat.append(input_outs["last_hidden_state"])
            encoder_output = torch.cat(to_cat, dim = 1)
            input_outs["last_hidden_state"] = encoder_output
            input_mask = input_mask[:, :thresh]
    ...
    def generate(
        ...
        **model_kwargs,
        ...
        if self.config.is_encoder_decoder and "encoder_outputs" not in model_kwargs:
            ...
            if "use_source" in model_kwargs.keys() and model_kwargs["use_source"] == False:
                model_kwargs["attention_mask"] = model_kwargs["attention_mask"][:, -model_kwargs["encoder_outputs"][0].shape[1]:]
            if "use_candidates" in model_kwargs.keys() and model_kwargs["use_candidates"] == False:
                model_kwargs["attention_mask"] = model_kwargs["attention_mask"][:, :model_kwargs["encoder_outputs"][0].shape[1]]
